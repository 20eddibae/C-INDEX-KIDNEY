...
def load_and_prepare_data():
    """Load standard results and KiTS19 data, then merge them."""
    
    # Load standard results (tumor radius method)
    standard_df = pd.read_csv('standard_results.csv')
    print(f"Loaded {len(standard_df)} standard C-index cases")
    ...
    # Select relevant columns from standard results
    standard_clean = standard_df[['case_id', 'c_index_standard', 'distance_mm', 'tumor_radius_mm']].copy()
    standard_clean.columns = ['case_id', 'standard_c_index', 'standard_distance_mm', 'standard_tumor_radius_mm']
    
    # Merge datasets on case_id
    merged_df = pd.merge(standard_clean, kits19_clean, on='case_id', how='inner')
    
    print(f"After merging: {len(merged_df)} cases with both standard and KiTS19 data")
...
def perform_regression_analysis(df):
    ...
    X = df_clean['standard_c_index'].values.reshape(-1, 1)
...
def create_visualizations(results, df):
    ...
    fig.suptitle('STANDARD C-Index vs KiTS19 Manual: Tumor Radius Method', fontsize=16, fontweight='bold')
    ...
    axes[0, 0].set_xlabel('Standard C-Index (Tumor Radius)')
    ...
    axes[0, 2].axhline(y=mean_diff - 1.96*std_diff, color='red', linestyle='--', label='-1.96 SD')
    axes[0, 2].set_xlabel('Mean of Standard and KiTS19 C-Index')
    axes[0, 2].set_ylabel('Difference (Standard - KiTS19)')
    axes[0, 2].set_title('Bland-Altman Plot')
    axes[0, 2].legend()
    axes[0, 2].grid(True, alpha=0.3)
    
    # 4. Distribution comparison
    axes[1, 0].hist(results['X'].flatten(), alpha=0.7, label='Standard', bins=20, color='blue')
    ...
    axes[1, 1].scatter(df['standard_distance_mm'], df['kits19_distance_mm'], alpha=0.6, color='orange')
    max_dist = max(df['standard_distance_mm'].max(), df['kits19_distance_mm'].max())
    axes[1, 1].plot([0, max_dist], [0, max_dist], 'k--', alpha=0.5)
    axes[1, 1].set_xlabel('Standard Distance (mm)')
    axes[1, 1].set_ylabel('KiTS19 Distance (mm)')
    axes[1, 1].set_title('Distance Comparison')
    axes[1, 1].grid(True, alpha=0.3)
    
    # 6. Tumor radius comparison
    axes[1, 2].scatter(df['standard_tumor_radius_mm'], df['kits19_tumor_radius_mm'], alpha=0.6, color='brown')
    max_radius = max(df['standard_tumor_radius_mm'].max(), df['kits19_tumor_radius_mm'].max())
    axes[1, 2].plot([0, max_radius], [0, max_radius], 'k--', alpha=0.5)
    axes[1, 2].set_xlabel('Standard Tumor Radius (mm)')
    axes[1, 2].set_ylabel('KiTS19 Tumor Radius (mm)')
    axes[1, 2].set_title('Tumor Radius Comparison')
    axes[1, 2].grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('standard_c_index_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def print_statistical_summary(results, df):
    ...
    print("\n" + "="*80)
    print("STANDARD C-INDEX vs KiTS19 MANUAL: TUMOR RADIUS METHOD")
    print("="*80)
    
    print(f"\nDataset Information:")
    print(f"  Total cases analyzed: {len(results['df_clean'])}")
    print(f"  Standard C-index range: {results['X'].min():.3f} - {results['X'].max():.3f}")
    print(f"  KiTS19 C-index range: {results['y'].min():.3f} - {results['y'].max():.3f}")
    
    print(f"\nRegression Results:")
    print(f"  Equation: KiTS19_C_index = {results['slope']:.3f} × Standard_C_index + {results['intercept']:.3f}")
    ...
    print(f"  Standard C-index - Mean: {np.mean(results['X']):.3f}, Std: {np.std(results['X']):.3f}")
...
def save_detailed_results(results, df):
    ...
    # Save to CSV
    detailed_df.to_csv('standard_detailed_comparison.csv', index=False)
    print(f"\nDetailed results saved to 'standard_detailed_comparison.csv'")
...
def main():
    ...
    print("  - standard_c_index_comparison.png (visualizations)")
    print("  - standard_detailed_comparison.csv (detailed data)")
